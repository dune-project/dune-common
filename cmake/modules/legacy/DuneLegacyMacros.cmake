...
macro(dune_project)
  ...
  # NOTE(SP): we now require cmake >= 3.13
  ...
  # NOTE(SP): autotools functionality only working with Makefiles. Should be removed!!!!!
  ...
  # NOTE(SP): not sure about those flags
  ...
  # NOTE(SP): This looks like a hack
  if(DUNE_USE_ONLY_STATIC_LIBS)
  ...
  # NOTE(SP): implemented in dune-common by target_compile_features()
  # include(CheckCXXFeatures)

  # NOTE(SP): This is a special test for abi::__cxa_demangle function. Should be moved to target_compile_feature
  # include(DuneCxaDemangle)

  # set include path and link path for the current project.
  # NOTE(SP): Can we workaround this by setting properties to some target?
  ...
  # NOTE(SP): Use the dune-common find_package(MPI) instead.
  # include(DuneMPI)
  ...
  # NOTE(SP): Should be moved to new build-system
  include(DuneDoc)

  # activate pkg-config
  # NOTE(SP): necessary for installing configuration of dune-module as PkgConfig .pc file
  ...
  # NOTE(SP): Why would you do somthing like this? Is this inherited from autotools?
...
macro(finalize_dune_project)
  # NOTE(SP): Can we remove the symlink feature?
  ...
  # NOTE(SP): Maybe we can add dependencies via find_dependency() here?
...
# fallback implementation of dune_add_library
function (dune_add_library)
  cmake_parse_arguments(DUNE_LIB
    "APPEND;NO_EXPORT;OBJECT"
    "COMPILE_FLAGS"
    "ADD_LIBS;SOURCES"
    ${ARGN})

  if (DUNE_LIB_APPEND OR DUNE_LIB_NO_EXPORT OR DUNE_LIB_OBJECT OR DUNE_LIB_COMPILE_FLAGS OR DUNE_LIB_ADD_LIBS OR DUNE_LIB_SOURCE)
    dune_legacy_add_library(${ARGN})
  else ()
    _dune_add_library(${ARGN})
  endif ()
...
macro(dune_legacy_add_library basename)
  # message(DEPRECATION "The function dune_legacy_add_library is deprecated. Use add_library instead!")
  ...
  if(DUNE_LIB_OBJECT)
    if(DUNE_LIB_${basename}_SOURCES)
      ...
      set_property(GLOBAL PROPERTY DUNE_LIB_${basename}_ADD_LIBS
        "${DUNE_LIB_ADD_LIBS};Dune::dune-common")
    ...
    target_link_libraries(${basename} Dune::dune-common)
    ...
    if(DUNE_BUILD_BOTH_LIBS)
      if(BUILD_SHARED_LIBS)
        ...
        target_link_libraries(${basename}-static Dune::dune-common)
        if(DUNE_LIB_COMPILE_FLAGS)
        ...
        target_link_libraries(${basename}-shared Dune::dune-common)
        if(DUNE_LIB_COMPILE_FLAGS)
...
endmacro(dune_legacy_add_library basename sources)
