...
from dune.utility import buffer_to_str, isString, reload_module
...
class Builder:
    def __init__(self, force=False, saveOutput=False):
        self.force = force
        self.dune_py_dir = module.get_dune_py_dir()
        ...
        if rank == 0:
            ...
            with Lock(os.path.join(self.dune_py_dir, 'lock-module.lock'), flags=LOCK_EX):
                module.make_dune_py_module(self.dune_py_dir)
                tagfile = os.path.join(self.dune_py_dir, ".noconfigure")
                if not os.path.isfile(tagfile):
                    module.build_dune_py_module(self.dune_py_dir)
                    logger.info('configured dune-py module')
                else:
                    logger.info('using pre configured dune-py module')
        comm.barrier()

        self.build_args = module.get_default_build_args()
    ...
    def compile(self, target='all'):
        cmake_command = module.get_cmake_command()
    ...
    def load(self, moduleName, source, pythonName):
        if not self.force: # no forced reload so chek if module already loaded
          try:
              return sys.modules["dune.generated." + moduleName]
          except KeyError:
              pass
        if rank == 0:
