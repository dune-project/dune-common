...
PRAGMA_RE = re.compile(r"^#pragma once\s*$", re.MULTILINE)
CLOSE_GUARD = "#endif // DUNE_USE_SYSTEM_PYBIND11"


def transform_header(path: Path, root: Path) -> tuple[bool, str, str | None]:
    text = path.read_text(encoding="utf-8")

    if "#if DUNE_USE_SYSTEM_PYBIND11" in text and CLOSE_GUARD in text:
        return False, "already_wrapped", None

    match = PRAGMA_RE.search(text)
    if not match:
        return False, "missing_pragma_once", None

    rel = path.relative_to(root).as_posix()
    replacement = (
        "#pragma once\n"
        "#if DUNE_USE_SYSTEM_PYBIND11\n"
        f"#include <pybind11/{rel}>\n"
        "#else"
    )
    updated = PRAGMA_RE.sub(replacement, text, count=1)

    if updated.endswith("\n"):
        updated += f"{CLOSE_GUARD}\n"
    else:
        updated += f"\n{CLOSE_GUARD}\n"
