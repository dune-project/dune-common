...
namespace Dune {
  ...
  public:
    ...
    FieldVector<T,m> mult_t (const FieldVector<T,n>& x) const
    {
      ...
      for (int j=0; j<n; j++)
        for (int i=0; i<m; i++) z[i] += matrix[i][j] * x[j];
      return z;
    }
  ...
  namespace HelpMat {
    ...
    static inline T determinantMatrix (const Mat<3,3,T> &matrix)
    {
      ...
      T t6  = matrix[0][0] * matrix[2][1];
      T t8  = matrix[1][0] * matrix[0][1];
      T t10 = matrix[2][0] * matrix[0][1];
      T t12 = matrix[1][0] * matrix[0][2];
      T t14 = matrix[2][0] * matrix[0][2];

      T det = (t4*matrix[2][2]-t6*matrix[1][2]-t8*matrix[2][2]+
               t10*matrix[1][2]+t12*matrix[2][1]-t14*matrix[1][1]);
      return det;
    }
