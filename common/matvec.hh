...
namespace Dune {
  ...
  {
    return HelpMat::determinatMatrix( *this);
  }
  ...
  namespace HelpMat {
    ...
    static inline T determinatMatrix (const Mat<row,col,T> &matrix)
    {
    ...
    template <typename T>
    static inline T determinatMatrix (const Mat<1,1,T> &matrix)
    {
      return matrix(0,0);
    }

    template <typename T>
    static inline T determinatMatrix (const Mat<2,2,T> &matrix)
    {
      T det = (matrix(0,0)*matrix(1,1) - matrix(0,1)*matrix(1,0));
      return det;
    }
    template <typename T>
    static inline T determinatMatrix (const Mat<3,3,T> &matrix)
    {
