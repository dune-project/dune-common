...
#ifndef DUNE_COMMON_KERNEL_VEC_SUBTRACT_HH
#define DUNE_COMMON_KERNEL_VEC_SUBTRACT_HH
  ...
  namespace Kernel {
    namespace vec {
      ...
      template<typename T1,
               ...
               typename size_type,
               size_type alignment>
      void subtract(T1* DUNE_RESTRICT x,
                    T2* DUNE_RESTRICT y,
...
#define DECLARE_KERNEL(T1,T2,I,alignment)                               \
      template                                                          \
      void subtract<T1,T2,I,alignment>(                                 \
        T1* DUNE_RESTRICT x,                                            \
        T2* DUNE_RESTRICT y,                                            \
        I n);
                      ...
                      size_type n)
        {
          ...
          for (size_type b = 0; b != n; ++b)
            for (size_type i = 0; i < block_size; ++i)
              x[b*block_size + i] -= y[b*block_size + i];
        }
...
#define DECLARE_KERNEL(T1,T2,I,alignment,block_size)                    \
        template                                                        \
        void subtract<T1,T2,I,alignment,block_size>(                    \
          T1* DUNE_RESTRICT x,                                          \
          T2* DUNE_RESTRICT y,                                          \
          I n);
...
#endif // DUNE_COMMON_KERNEL_VEC_SUBTRACT_HH
