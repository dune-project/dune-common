...
#ifndef DUNE_COMMON_KERNEL_VEC_INFINITY_NORM_HH
#define DUNE_COMMON_KERNEL_VEC_INFINITY_NORM_HH
  ...
  namespace Kernel {
    namespace vec {
      ...
      template<typename T1,
               ...
               size_type alignment>
      T1
      infinity_norm(T1* DUNE_RESTRICT x,
                      ...
                      size_type n)
        {
          ...
          for (size_type b = 0; b != n; ++b)
            for (size_type i = 0; i < block_size; ++i)
              res = std::max(res,std::abs(x[b*block_size + i]));
          return std::move(res);
        }
...
#define DECLARE_KERNEL(T1,I,alignment,block_size)                       \
        template                                                        \
        T1                                                              \
        infinity_norm<T1,I,alignment,block_size>(                       \
          T1* DUNE_RESTRICT x,                                          \
          I n);
...
#endif // DUNE_COMMON_KERNEL_VEC_INFINITY_NORM_HH
