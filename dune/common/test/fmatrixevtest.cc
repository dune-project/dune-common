...
template <typename T> bool test1() {
  ...
  Dune::FieldVector<T, 3> sol;
  {
    /*
      ...
      optimize(eigenvalues(A));
    */
    using std::sqrt;
    using std::pow;
    auto t1 = sqrt(T(3));
    auto t2 = pow(std::complex<T>{0, 91} / (2 * pow(t1, T(3))) + 3031 / T(54),
                  1 / T(3));
    auto t3 = (std::complex<T>{0, -t1} - T(1)) / T(2);
    auto t4 = T(1) / t2;
    auto t5 = (std::complex<T>{0, t1} - T(1)) / T(2);

    sol = {std::real(T(133) * t4 * t5 / T(9) + t2 * t3 + 11 / T(3)),
           std::real(T(133) * t3 * t4 / T(9) + t2 * t5 + 11 / T(3)),
           std::real(T(133) * t4 / T(9) + t2 + 11 / T(3))};
...
template <typename T> bool test3b() {
  ...
  Dune::FieldVector<T, 3> sol;
  {
    ...
    */
    using std::sqrt;
    using std::pow;
    sol = {2 - pow(T(2), 3 / T(2)), 2 + pow(T(2), 3 / T(2)), 0};
    std::sort(sol.begin(), sol.end());
  }
  bool ret = true;
  {
