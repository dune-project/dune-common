...
namespace Dune
{
  ...
  */
  ...
  namespace Impl
  {
    ...
    private:
      static constexpr bool is_bidirectional = std::is_base_of<std::bidirectional_iterator_tag, iterator_category>::value;
      static constexpr bool is_random_access = std::is_base_of<std::random_access_iterator_tag, iterator_category>::value;

    public:
      using reference = decltype(std::declval<F>()(*(std::declval<I>())));
      using value_type = std::decay_t<reference>;
      using pointer = PointerProxy<value_type>;
      using difference_type = std::enable_if_t<is_random_access, typename std::iterator_traits<I>::difference_type>;
      ...
      std::enable_if_t<is_bidirectional, TransformedRangeIterator&>
      operator--() noexcept {
        --it_;
        return *this;
      }
      ...
      std::enable_if_t<is_random_access, TransformedRangeIterator&>
      ...
      std::enable_if_t<is_random_access, bool>
      operator<(const TransformedRangeIterator& other) noexcept {
        return it_<other.it_;
      }
      ...
      std::enable_if_t<is_random_access, reference>
      operator[](difference_type n) noexcept {
        return (*f_)(*(it_+n));
      }
