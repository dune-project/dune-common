...
namespace Dune
{
  ...
  private:
     ...
     */
    using SmartPointerAllocator = std::allocator_traits<A>::template rebind_alloc< std::shared_ptr< std::array<MemberType,chunkSize_> > >;

    /**
     * @brief The allocator for the fixed array.
     */
    using ArrayAllocator = std::allocator_traits<A>::template rebind_alloc< std::array<MemberType,chunkSize_> >;
  ...
  public:
    ...
    using reference = A::value_type &;

    using const_reference = A::value_type const&;
